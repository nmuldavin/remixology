...
urlpatterns = [
    ...
    url(r'^collections/$', views.DirectoryList.as_view(), name='directory_list'),
    url(r'^(?P<user_directory>[-\w]+)/cocktails/addcocktail/$', views.AddCocktail.as_view(), name='add_cocktail'),
    url(r'^(?P<user_directory>[-\w]+)/cocktails/(?P<cocktail_slug>[-\w]+)/delete_cocktail/$', views.DeleteCocktail.as_view(), name='delete_cocktail'),
    ...
    url(r'^(?P<user_directory>[-\w]+)/cocktails/(?P<cocktail_slug>[-\w]+)/(delete_recipe)/(?P<rank>[0-9]+)/$', views.DeleteRecipe.as_view(), name='delete_recipe'),
